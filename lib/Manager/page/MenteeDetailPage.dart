// lib/Manager/page/mentee_detail_page.dart
...
import 'package:nail/Manager/page/tabs/mentee_manage_tab.dart'; // for MenteeEntry
...
class _MenteeDetailPageState extends State<MenteeDetailPage> {
  ...
  Future<void> _editMentee() async {
    final result = await Navigator.of(context).push<MenteeEditResult>(
      MaterialPageRoute(builder: (_) => MenteeEditPage(initial: _mentee)),
    );
    if (result == null) return;
    if (result.deleted) {
      if (!mounted) return;
      Navigator.of(context).pop(); // 삭제 시 상세 화면 닫기
      return;
    }
    if (result.mentee != null && mounted) {
