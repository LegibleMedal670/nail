...
enum MentorSort {
  ...
  menteeDesc,     // 멘티수 많은순
  menteeAsc,      // 멘티수 적은순
  fastGraduate,   // 졸업 빨리순(일수 적은 순)
  scoreDesc,      // 평균 점수 높은순
}

class MentorManageTab extends StatefulWidget {
  /// KPI/차트용 숫자들
  final List<int> menteesPerMentor; // 멘토별 멘티수
  final List<int> pending7d;        // 최근 7일 평가대기 수(일별)
  final List<int> pending28d;       // 최근 4주 평가대기 수(주별)
...
class _MentorManageTabState extends State<MentorManageTab> {
  ...
  int get _totalMentees =>
      widget.menteesPerMentor.fold(0, (a, b) => a + b);
  double get _avgPerMentor =>
      _totalMentors == 0 ? 0 : _totalMentees / _totalMentors;
  int get _minPerMentor =>
      widget.menteesPerMentor.isEmpty
          ? 0
  ...
  List<Mentor> get _sortedMentors {
    final list = List<Mentor>.from(widget.mentors);
  ...
  Future<void> _showMentorSortSheet() async {
  ...
  Widget build(BuildContext context) {
    final changeColor =
    _changeRate >= 0 ? Colors.green.shade600 : Colors.red.shade600;
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const Spacer(),
                TextButton.icon(
                  onPressed: _showMentorSortSheet,
              ...
              return MentorTile(
                ...
                onTap: () {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('${m.name} 상세 보기')),
                  );
                },
              );
            },
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Icon(Icons.groups_2_outlined, color: UiTokens.primaryBlue, size: 22),
          const SizedBox(height: 8),
          Text(
            title,
            style: const TextStyle(
