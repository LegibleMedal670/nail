...
class MentorJournalHistoryPage extends StatefulWidget {
  ...
  const MentorJournalHistoryPage({
    super.key,
  ...
  @override
  State<MentorJournalHistoryPage> createState() =>
      _MentorJournalHistoryPageState();
}

class _MentorJournalHistoryPageState extends State<MentorJournalHistoryPage> {
  ...
  Future<void> _loadMonth() async {
    ...
    try {
      final rows = await SupabaseService.instance.mentorListMenteeJournalsByMonth(
  ...
  Future<void> _openDetailFor(DateTime date) async {
    ...
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => _MentorHistoryDetailPage(
          journalId: journalId,
...
class _MentorCalendarGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final int startWeekday = firstDay.weekday % 7; // 월=1, 일=7 → 0~6
    final int totalDays = lastDay.day;
    final int totalCells =
        ((startWeekday + totalDays + 6) ~/ 7) * 7; // 6주까지 커버
        ...
        Expanded(
          child: GridView.builder(
            ...
            itemBuilder: (_, index) {
              ...
              final Color dotColor = hasJournal
                  ? statusColor(status)
                  : UiTokens.title.withOpacity(0.15);

              return _MentorDayCell(
                date: date,
                isToday: isToday,
...
class _MentorDayCell extends StatelessWidget {
  final DateTime date;
  final bool isToday;
  ...
  Widget build(BuildContext context) {
    ...
    final Color textColor = hasJournal
        ? UiTokens.title
...
class _MentorHistoryDetailPage extends StatefulWidget {
  ...
  const _MentorHistoryDetailPage({
  ...
  @override
  State<_MentorHistoryDetailPage> createState() =>
      _MentorHistoryDetailPageState();
}

class _MentorHistoryDetailPageState extends State<_MentorHistoryDetailPage> {
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    try {
      final data =
          await SupabaseService.instance.getJournalDetail(journalId: widget.journalId);
      if (data != null) {
      ...
      debugPrint('MentorHistoryDetail load error: $e');
    } finally {
      if (mounted) {
        setState(() {
          _loading = false;
  ...
  String _fmtTitleDate() {
    ...
    if (d == null) return '${widget.menteeName} 일지 상세';
    return '${widget.menteeName} · ${d.year}.${d.month.toString().padLeft(2, '0')}.${d.day.toString().padLeft(2, '0')} 일지';
  }
