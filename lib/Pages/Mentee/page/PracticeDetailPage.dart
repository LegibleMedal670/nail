// lib/Pages/Mentee/page/AdminPracticeDetailPage.dart
...
class _PracticeDetailPageState extends State<PracticeDetailPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      detail = await _api.menteePracticeSetDetail(
        setId: widget.setId,
        limit: 30,
      );
      loading = false;
      ...
      body: ListView(
        ...
        children: [
          if (d != null) ...[
            ...
            // 레퍼런스 이미지(간단 텍스트로)
            if (d['set_reference_imgs'] != null)
              Text('예시: ${(d['set_reference_imgs'] as List).length}개'),
            const SizedBox(height: 12),
            ...
            if (d['history'] is List && (d['history'] as List).isNotEmpty)
              ...((d['history'] as List).map(
                (h) => _historyTile(h as Map<String, dynamic>),
              ))
            else
              Text(
                '기록 없음',
  ...
  Widget _statusBlock(Map<String, dynamic> d) {
    final status = SupabaseService.instance.practiceStatusLabel(
      d['current_status'] as String?,
    );
  ...
  Widget _historyTile(Map<String, dynamic> h) {
    final label = SupabaseService.instance.practiceStatusLabel(
      h['status'] as String?,
    );
